...
Avaliação usando Random Forest com Repeated Stratified Hold-Out (30 repetições).

Cenários avaliados:
  1) RF - Baseline (sem ajuste de peso)
  2) RF - SMOTE
  3) RF - Cost-sensitive (class_weight="balanced")
  4) RF - Engenharia de atributos não supervisionada (Cenário Proposto)
"""
...
print("Iniciando pipeline de avaliação - Random Forest (4 Cenários)")
print("-" * 60)
...
# df = pd.concat([
#     pd.DataFrame(df.data,   columns=range(df.data.shape[1])),
#     pd.DataFrame(df.target, columns=[df.data.shape[1]])
# ], axis=1)
# df.columns = [f"Feature{i}" for i in df.columns[:-1]] + ['Target']
...
# Nota: O PowerTransformer é mantido no RF por consistência metodológica com 
# o pipeline da LR, garantindo a mesma entrada padronizada no UnsupervisedFeaturizer.
...
rf_none_params     = dict(n_estimators=200, random_state=42, class_weight=None,       n_jobs=-1)
rf_balanced_params = dict(n_estimators=200, random_state=42, class_weight='balanced', n_jobs=-1)
...
print("\nExecutando RandomizedSearch no cenário 4 (apenas hiperparâmetros do Unsup)...")
_model_ft = RandomForestClassifier(**rf_none_params)
...
SCENARIOS = [
    ('RF (cw=None)',        make_pipe_orig,   rf_none_params,     '1-Orig (cw=None)'),
    ('RF (cw=None)+SMOTE',  make_pipe_smote,  rf_none_params,     '2-Orig+SMOTE'),
    ('RF (balanced)',       make_pipe_orig,   rf_balanced_params, '3-Orig (cw=balanced)'),
    ('RF+Unsup',            make_pipe_full,   rf_none_params,     '4-Orig+Unsup'),
]
...
for rep_idx, (X_tr, X_te, y_tr, y_te) in enumerate(all_splits):
    ...
    for model_name, pipe_fn, rf_params_dict, feat_label in SCENARIOS:
        model = RandomForestClassifier(**rf_params_dict)
        pipe  = pipe_fn(model)
        pipe.fit(X_tr, y_tr)
...
combined_holdout_df.to_excel('figures_all_rf/results_holdout_completo_v5_rf.xlsx', index=False)

# --- 9. Feature Importance (Cenário 4) ---

def _build_fi(pipe_fn, label):
    """Extrai as importâncias MDI do RandomForest normalizadas."""
    model  = RandomForestClassifier(**rf_none_params)
    pipe   = pipe_fn(model)
    ...
    n_imp  = len(fitted.feature_importances_)
    if len(names) != n_imp:
        names = [f'feature_{i}' for i in range(n_imp)]
        
    raw  = fitted.feature_importances_
    norm = raw / raw.sum()
...
fi_c4.to_excel('figures_all_rf/feature_importance_c4_v5_rf.xlsx', index=False)
st_df_c4.to_excel('figures_all_rf/feature_importance_by_group_c4_v5_rf.xlsx', index=False)
...
# 10.1 Feature Importance Top 20 (MDI)
...
ax.set_xlabel('Importância RF (MDI normalizado)', fontsize=10)
ax.set_title('C4: Dados com engenharia de atributos - Random Forest')
ax.invert_yaxis()
...
plt.tight_layout()
plt.savefig('figures_all_rf/feature_importance_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('figures_all_rf/feature_importance_bar_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()

# 10.3 Curva de Importância Cumulativa
fig, ax = plt.subplots(1, 1, figsize=(10, 5))
fi_sorted_cum = fi_c4.sort_values('Importance', ascending=False).reset_index(drop=True)
cumsum    = fi_sorted_cum['Importance'].cumsum()
thresh_80 = (cumsum >= 0.80).idxmax() + 1
thresh_95 = (cumsum >= 0.95).idxmax() + 1
ax.plot(range(1, len(fi_sorted_cum) + 1), cumsum, color='#373B42', linewidth=2.2)
ax.axhline(0.80, color='#5C7A99', linestyle='--', linewidth=1.4, label=f'80% ({thresh_80} feat.)')
ax.axhline(0.95, color='#C46D61', linestyle='--', linewidth=1.4, label=f'95% ({thresh_95} feat.)')
ax.set(xlabel='Número de Features (ordenadas por importância)', ylabel='Importância Cumulativa (MDI)', xlim=(1, len(fi_sorted_cum)), ylim=(0, 1.05))
ax.set_title('C4: Curva de Importância Cumulativa - Random Forest')
ax.legend(fontsize=9)
plt.tight_layout()
plt.savefig('figures_all_rf/feature_importance_cumulative_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.savefig('figures_all_rf/boxplot_stripplot_metrics_repeated_ho_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('figures_all_rf/pr_curve_matrix_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('figures_all_rf/sep_S4_summary_table_holdout_rf.png', dpi=300, bbox_inches='tight')
plt.close(fig)
...
plt.tight_layout()
plt.savefig('figures_all_rf/precision_recall_threshold_matrix_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close()
...
pd.DataFrame(_sig_records).to_excel('figures_all_rf/stat_sig_results_v5_rf.xlsx', index=False)
...
_fig14a.suptitle('Significância Estatística — t de Student Pareado + Bonferroni - Random Forest', fontsize=11, fontweight='bold', y=1.04)
plt.tight_layout()
plt.savefig('figures_all_rf/stat_sig_table_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close(_fig14a)
...
_fig14b.suptitle('Forest Plot — Diferença Média C4 vs Baseline - Random Forest', fontsize=12, fontweight='bold', y=1.04)
plt.tight_layout()
plt.savefig('figures_all_rf/stat_sig_forest_v5_rf.png', dpi=300, bbox_inches='tight')
plt.close(_fig14b)

print("Processamento concluído. Arquivos gerados na pasta figures_all_rf/")